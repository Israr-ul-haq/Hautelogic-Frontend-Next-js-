    ...
    <div className="fashiontabssection pt-md-80 pt-0 pb-55 pb-md-80">
              ...
              {data?.data?.map((item, index) => {
                return (
                  <Link
                    ...
                    offset={-100}
                  >
                    {item?.attributes?.Apparel_Management_Tab_Title}
                  </Link>
                );
              })}
            </div>
      ...
      {data?.data?.map((item, index) => {
        return (
          <TabContent
            key={item?.id}
            title={item?.attributes?.Apparel_Management_Title}
            subtitle={item?.attributes?.Apparel_Management_Description}
            image={
              item?.attributes?.Apparel_Management_Image?.data?.attributes?.url
            }
