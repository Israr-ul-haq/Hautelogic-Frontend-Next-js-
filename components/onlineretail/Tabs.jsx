    ...
    <div className="fashiontabssection pt-md-80 pt-0 pb-55 pb-md-80">
              ...
              {data?.data?.map((item, index) => {
                return (
                  <Link
                    ...
                    offset={-100}
                  >
                    {item?.attributes?.Online_Retail_Tab_Title}
                  </Link>
                );
              })}
            </div>
      ...
      {data?.data?.map((item, index) => {
        return (
          <TabContent
            key={item?.id}
            title={item?.attributes?.Online_Retail_Title}
            subtitle={item?.attributes?.Online_Retail_Description}
            image={
              item?.attributes?.Online_Retail_Image?.data?.attributes?.url
            }
